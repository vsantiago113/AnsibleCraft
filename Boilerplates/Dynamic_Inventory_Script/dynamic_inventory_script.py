...
class AnsibleDynamicInventory:
    ...
    def add_hosts(self, *, hosts: Optional[List[str]] = None, group: str, vars: Optional[Dict] = None, group_vars: Optional[Dict] = None) -> None:
        """
    ...
    def add_host(self, *, host: str, group: Optional[str], vars: Optional[Dict] = None, group_vars: Optional[Dict] = None) -> None:
        """
    ...
    def add_host_to_group(self, *, host: str, group: str, vars: Optional[Dict] = None, group_vars: Optional[Dict] = None) -> None:
        """
        Add an existing host to a group, or create and add the host if it doesn't exist, with optional host and group variables.
    ...
    def _ensure_group_exists(self, *, group: str, group_vars: Optional[Dict] = None) -> None:
        ...
        if group_vars:
            self.add_group_vars(group=group, vars=group_vars)

    def add_group_vars(self, *, group: str, vars: Dict) -> None:
        """
        Add variables to a group.

        Args:
            group (str): The group to add the variables to.
            vars (Dict): A dictionary of variables to associate with the group.
        """
